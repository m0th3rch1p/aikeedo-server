...
#[Route(path: '/stabilityai', method: RequestMethod::POST)]
class SaveStabilityAiOptionsRequestHandler extends OptionsApi implements
    RequestHandlerInterface
{
    ...
    private function validateRequest(ServerRequestInterface $req): array
    {
        ...
        $this->validator->validate(
            $payload,
            [
                'stabilityai.api_key' => 'required|string'
            ]
        );

        $this->validateOpenApiKey($payload['stabilityai']['api_key']);
    ...
    private function validateOpenApiKey(string $key): bool
    {
        $req = $this->requestFactory
            ->createRequest('GET', 'https://api.stability.ai/v1/engines/list')
